...
import { type Attributes, type Children, LinkPresentationArc } from '../../model/link/presentation-arc.ts';
import type { XmlElement } from '../../model/xml/element.ts';
import { BaseFactory } from '../base.ts';

export class LinkPresentationArcFactory extends BaseFactory<LinkPresentationArc, Children, Attributes> {
  ...
  override async map(element: XmlElement, context: XmlMapperContext) {
    ...
    return success(new LinkPresentationArc(element, result.data.children, result.data.attributes));
  }
}
