...
import { type Attributes, type Children, LinkLabelArc } from '../../model/link/label-arc.ts';
import type { XmlElement } from '../../model/xml/element.ts';
import { BaseFactory } from '../base.ts';

export class LinkLabelArcFactory extends BaseFactory<LinkLabelArc, Children, Attributes> {
  ...
  override async map(element: XmlElement, context: XmlMapperContext) {
    ...
    return success(new LinkLabelArc(element, result.data.children, result.data.attributes));
  }
}
