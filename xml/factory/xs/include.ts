...
import { type Attributes, type Children, XsInclude } from '../../model/xs/include.ts';
import { BaseFactory } from '../base.ts';

export class XsIncludeFactory extends BaseFactory<XsInclude, Children, Attributes> {
  ...
  override async map(element: XmlElement, context: XmlMapperContext) {
    ...
    return success(new XsInclude(element, result.data.children, result.data.attributes));
  }
}
