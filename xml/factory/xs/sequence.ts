...
import { type Attributes, type Children, XsSequence } from '../../model/xs/sequence.ts';
import { BaseFactory } from '../base.ts';

export class XsSequenceFactory extends BaseFactory<XsSequence, Children, Attributes> {
  ...
  override async map(element: XmlElement, context: XmlMapperContext) {
    ...
    return success(new XsSequence(element, result.data.children, result.data.attributes));
  }
}
