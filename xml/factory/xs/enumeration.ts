...
import { type Attributes, type Children, XsEnumeration } from '../../model/xs/enumeration.ts';
import { BaseFactory } from '../base.ts';

export class XsEnumerationFactory extends BaseFactory<XsEnumeration, Children, Attributes> {
  ...
  override async map(element: XmlElement, context: XmlMapperContext) {
    ...
    return success(new XsEnumeration(element, result.data.children, result.data.attributes));
  }
}
