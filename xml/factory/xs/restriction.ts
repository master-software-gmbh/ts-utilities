...
import { type Attributes, type Children, XsRestriction } from '../../model/xs/restriction.ts';
import { BaseFactory } from '../base.ts';

export class XsRestrictionFactory extends BaseFactory<XsRestriction, Children, Attributes> {
  ...
  override async map(element: XmlElement, context: XmlMapperContext) {
    ...
    return success(new XsRestriction(element, result.data.children, result.data.attributes));
  }
}
